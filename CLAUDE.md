...
**Probe cheap before any long ingest; fetch narrow; verify key stability (locked 2026-06-13):** a <1-min check (is the key stable across refreshes? one-page timing × pages = total?) comes BEFORE launching any multi-minute ingest/backfill — never run 50 min to find out at minute 51 it was wrong. An ingest pipeline `$select`s ONLY the columns its normalizer reads, at the largest `$top` the API honors (FEMA NFIP: ~70 cols × $top=500 ≈ 50 min + drop-prone → 16 cols × $top=10000 ≈ 3 min). To fix a _column_ you must `UPDATE` by a key that is STABLE across refreshes — OpenFEMA regenerates `id` every rebuild (a stored id returns EMPTY live), so there is no stable key → full `replace` is the only correct way (merge/backfill impossible, not lazy). Detail: THE BIBLE §0.1.
