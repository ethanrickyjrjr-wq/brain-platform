"""psycopg DB layer for market_aggregates — idempotent time-series upsert of aggregate rows.

Append-with-captured_date (never a destructive replace, so the Gate-4 guard is a no-op here): each
run inserts this period's rows; a same-period re-run MERGEs on the conflict key. History accumulates;
the consuming brain reads the latest captured_date. Mirrors listing_lifecycle/distill.py's connection
+ executemany pattern (psycopg + DATABASE_URL / .dlt/secrets.toml)."""
