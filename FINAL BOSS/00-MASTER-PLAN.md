...
| `project_feed` table — kinds: **outside-action** (BUILT, live) · data-change (cron — PR2) · engagement · external-event · platform-feature | P3 | P2 | situational-prompt fuel; `outside-action` emitted at claim/import **birth**; "platform-feature" = site agent announcing new charts/features. Migration `docs/sql/20260617_project_feed.sql` (RLS owner-all, `dedup_key` UNIQUE, `project_id` NULLABLE for Tier-2 scope-keyed rows) |
