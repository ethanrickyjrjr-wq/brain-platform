    ...
    <main className="min-h-dvh bg-[#0A1419] text-[#F0EDE6]">
      ...
      <section className="flex min-h-dvh flex-col items-center justify-center px-6">
        ...
        <p className="mt-6 max-w-xl text-center text-lg text-[#B8B4A8]">
          {statsData.corridors_tracked} corridors. {statsData.swfl_zips} ZIP
          codes. {statsData.flood_records.toLocaleString()} flood records. One
          verified answer.
        </p>
        <p className="mt-4 font-mono text-xs text-[#807E76]">
          Confidence: {statsData.brain_confidence}% ·{" "}
          {brainData.freshness_token}
        </p>
      </section>
      ...
      <section className="border-t border-[#22414F] px-6 py-24">
        <div className="mx-auto max-w-2xl">
          ...
          <p className="text-xl leading-relaxed text-[#F0EDE6]">
            {brainData.conclusion}
          </p>
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mt-6 grid grid-cols-2 gap-4 md:grid-cols-3">
            {brainData.key_metrics.map((m) => (
              <div
                key={m.label}
                className="rounded border border-[#22414F] bg-[#152832] p-5"
              >
                <p className="text-xs uppercase tracking-wider text-[#807E76]">
                  {m.label}
                </p>
