...
| `project_feed` table — kinds: **outside-action** (BUILT, live) · **data-change** (cron BUILT, HELD PR2) · engagement · external-event · platform-feature | P3 | P2 | situational-prompt fuel; `outside-action` emitted at claim/import **birth**; **data-change** = daily cron (`scripts/project-feed/change-detection.mts`) writing Tier-2 scope-keyed rows when a catalog-resolvable per-ZIP brain metric (`housing-swfl.median_sale_price`, `rentals-swfl.rent_index_latest`) moves — material gate `reconcileMetric`, the feed row IS the snapshot, `dedup_key=datachange:<user>:<zip>:<brain>:<token>`; "platform-feature" = site agent announcing new charts/features. Migration `docs/sql/20260617_project_feed.sql` (RLS owner-all, `dedup_key` UNIQUE, `project_id` NULLABLE for Tier-2 scope-keyed rows) |
| `writeFeed` / `readProjectFeed` / `markFeedSeen` (`lib/project/feed.ts`) | P3 | P2 | the feed seams — P3 writes (service-role, never-throws, `dedup_key` ON CONFLICT DO NOTHING); P2's `digest.ts` reads via `readProjectFeed(projectId, projectScopeSet(items))` (Bound ∪ Tier-2 scope-match, cookie/RLS) |
| `projectScopeSet(items)` (`lib/project/project-scope.ts`) + `identityKeyForItem(item)` (`lib/project/identity-key.ts`) | P3 (`project-scope`) · P2 (`identity-key`, reused) | P2,P3 | late-binding scope set for `readProjectFeed`; `payload.identityKey` IS the cross-project overlap key (`cross-project-index.ts` consumes the same fn) |
| `ProjectDigest.feedSignals` (`lib/project/digest.ts`) + feed candidate (`prompt-engine.ts`) | P3 wires into P2 (additive) | (UX) | feed signals folded into the digest (+ `computeRev` basis) + ONE capped prompt ranked between `freshData`/`staleMetric`; dismiss seam = `markFeedSeen`/PATCH `seen_feed_ids` (button DEFERRED — check `piece3_dismiss_ui`) |
