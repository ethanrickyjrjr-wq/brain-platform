...
> **Note:** The embedding strategy makes two types of API calls that need different model types: - **Embedding calls** (text → vector) require an embedding model like `text-embedding-3-small` - **Query expansion** (chat completion) requires a chat model like `gpt-4o-mini`
