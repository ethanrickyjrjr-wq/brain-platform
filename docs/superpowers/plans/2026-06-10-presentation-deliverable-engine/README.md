...
| 6 | Brand theming | `phase-6-brand-theming__SONNET.md` | Sonnet | 2a, 3 | ✅ 2026-06-11 (local) — `lib/deliverable/brand-theme.ts` (`extractBrandTheme`/`toChartTheme` pure fns); `brand-theme.test.ts` 10 tests; theme injected into every frame `ChartSpec` at render time in `/p/[id]` (no rebuild required); `FrameRenderer` wraps with `--chart-primary`/`--chart-accent` CSS custom props; `renderBranding()` shows logo from `branding.logo_url` + primary-tinted border; brand accent bar in page chrome (screen + PDF). 136 deliverable tests, tsc 0. |
