...
3. **Wire the cron to broadcast.** Today `build-digest.mts:198` hardcodes the footer `unsubscribeUrl` to `${SITE_URL}/unsubscribe?token=phase1-static` (a dead Phase-1 placeholder; fine for the internal hello@ send only). For broadcast mode, pass `unsubscribeUrl="{{{RESEND_UNSUBSCRIBE_URL}}}"` into `<DigestEmail>` so Resend manages per-recipient unsubscribe, then POST `{subject, html}` to `/api/email/broadcast` (bearer) instead of `emails.send`. Start `send:false` (draft → review in Resend dashboard), flip to `send:true` when confident.
   - **VERIFY at wiring time — the token must survive `render()` as a literal `{{{RESEND_UNSUBSCRIBE_URL}}}`.** Two encoding traps: (a) dropping it into a JSX **text node** entity-encodes the braces to `&#123;&#123;&#123;…`; (b) putting it through React Email's `<Link href={…}>` can URL-normalize/percent-encode it to `%7B%7B%7B…`. **Both break Resend's substitution.** Fix: render that one footer slot as **raw HTML** — `<td dangerouslySetInnerHTML={{ __html: '<a href="{{{RESEND_UNSUBSCRIBE_URL}}}" style="color:#888">Unsubscribe</a>' }} />` — NOT through `<Link>` or a JSX text node. The token is opaque to React that way and reaches Resend intact. After rendering, grep the HTML for the literal string to confirm. (The broadcast guard rejects HTML missing the literal token, so any encoded form fails closed — you'll catch it before a send either way.)
