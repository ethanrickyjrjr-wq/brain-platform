...
// 1. Stash any uncommitted tracked changes so rebase has a clean tree.
const dirty = sh("git diff --name-only");
const stashed = dirty.length > 0;
if (stashed) {
  console.log("Stashing uncommitted changes...");
  execSync("git stash push -m safe-push-stash", {
    cwd: ROOT,
    stdio: "inherit",
  });
}
...
function rebase() {
  try {
    ...
    if (stashed) execSync("git stash pop", { cwd: ROOT, stdio: "inherit" });
...
// 3. Rebase your commits on top of what just landed.
console.log("Rebasing onto origin/main...");
rebase();

// 4. Show exactly what's going.
...
// 5. Push — retry up to 3 times if another Claude beat us to origin.
